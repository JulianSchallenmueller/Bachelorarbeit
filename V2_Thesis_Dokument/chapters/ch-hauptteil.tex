...
\textbf{Measured Service}: Cloud Systeme kontrollieren und optimieren Ressourcennutzung
 automatisch mithilfe eines Mess-Systems das auf einer abstrakten Ebene den
...
eingeschränkte Kontrolle über bestimmte Komponenten, beispielsweise
Firewalls, haben.
...
Regierungsorganisation oder einer Kombination aus diesen und wird auch von der selben Organisation On-Premises betrieben. 
...
Mitgliedern dieser Gemeinschaft, einer Drittpartei und kann Off- oder On-Premises betrieben werden.
...
reduziertes Risiko dar\cite[S.23]{lisdorf-cpbasics}.\\
...
werden\cite[S.24]{lisdorf-cpbasics}.\\
...
Anbieter\cite[S.24-25]{lisdorf-cpbasics}.
...
als auch sinkende Nachfrage reagiert werden kann\cite[S.27]{lisdorf-cpbasics}.
...
mit minimaler Unterbrechung aufrecht zu erhalten\cite[S.28]{lisdorf-cpbasics}.
...
Schutz vor Denial-of-Service Angriffen durch ihre \enquote{natürliche} Skalierbarkeit\cite[S.25]{lisdorf-cpbasics}.

\subsubsection{Herausforderungen und Risiken}

\textbf{Netzwerkabhängigkeit}: Da der Zugriff auf Cloud Dienste über das
Internet erfolgt, entsteht dadurch entsprechend auch eine hohe Abhängigkeit. Stabile und schnelle Netzwerkanbindung ist eine kritische Voraussetzung für effektives Arbeiten\cite[S.737]{net-dep}, bei lokal gehosteten Systemen ist diese Abhängigkeit
...
Gründe die gegen Cloud Computing sprechen\cite[S.737]{net-dep}, besonders im Fall der Nutzung einer
...
entstehen als in einem vergleichbaren Zeitraum davor. Nicht zuletzt stellt die Migration komplexer Systeme häufig eine große technische und dadurch auch finanzielle Herausforderung dar\cite[S.321]{migration-cost}.
...
Softwareentwicklung\cite[S.4]{morris-iac}.
...
bestehenden Systeme jedoch nicht unbedingt einfacher\cite[S.3]{morris-iac}, die Anwendung von
...
\begin{table}[H]
	...
	\centering
	\caption{\enquote{Iron vs Cloud Age}\cite[S.3]{morris-iac}}
	\centering
...
sind monolitisch, die Konfiguration erfolgt eher mithilfe von GUI-gesteuerten Programmen oder direkter physischer Interaktion, zum Beispiel wenn ein neuer Server
...
starken Configuration Drift, der ansonsten oftmals stattfindet\cite[S.17-18]{morris-iac}.
...
wie leicht ersetzbares Vieh behandelt werden\cite[S.16]{morris-iac}.
...
\textbf{Höhere Zuverlässigkeit und Sicherheit durch schnelle Updates}: Veränderungen stellen die größte Gefahr für ein Produktionssystem dar, gleichzeitig sind Veränderungen aber unvermeidbar, um die Sicherheit des Systems zu gewährleisten und es zu verbessern. Durch die Fähigkeit, Veränderungen schnell und dennoch zuverlässig durchzuführen, wird die Reaktionszeit auf unvorhergesehene Ereignisse und Anforderungen wesentlich verkürzt\cite[S.4-5]{morris-iac}.
...
einen sicheren und zuverlässigen Betrieb zu gewährleisten. Neue Features in bestehender Software kann neue Infrastruktur, zum Beispiel in Form einer zusätzlichen Datenbank, notwendig machen oder eine veränderte Konfiguration
...
Können die Kapazitäten automatisiert erweitert werden, wird ein Zusammenbruch des Services auf einfache Art verhindert\cite[S.4-5]{morris-iac}.

\enquote{A fundamental truth of the Cloud Age is: Stablity comes from making changes.}\cite[S.5]{morris-iac}
...
Ansatz werden viele der Vorteile von IaC jedoch verwirkt. Ein bestehendes, komplexes Infrastruktur-System zu automatisieren, ist eine deutlich größere Herausforderung als ein System von Grund auf mit dem Gedanken an Automatisierung aufzubauen\cite[S.6]{morris-iac}.
...
\begin{figure}[H]
	...
	\caption{Verhältnis von Geschwindigkeit und Qualität\cite[S.8]{morris-iac}}
...
diesen damit auch dokumentiert\cite[S.10]{morris-iac}.
...
einzubauen\cite[S.10]{morris-iac}.
...
den Betrieb wieder herzustellen\cite[S.11]{morris-iac}.
...
(\texttt{google\_compute\_instance}), ist mit einem Beispiel und der zugehörigen
...
versioniert, geteilt und wiederverwendet werden können. Terraform Module können auf einer Public Module Registry \footnote{Vergleichbar mit der npm-Registry von Node.js} zur allgemeinen Verfügung bereitgestellt und von der breiteren Terraform-Community wiederverwendet werden. Terraform unterstützt über Plug-Ins, die Terraform Provider, ein große Anzahl an Plattformen.
...
jeder der in der ISO 25010 aufgelisteten Punkte sinnvoll angewandt werden.
...
\begin{figure}[H]
	...
	\caption{Überblick ISO 25010\cite{iso-25010}}
...
\begin{itemize}
	...
	\item \textbf{Functional Completeness}: Grad der Vollständigkeit, mit welcher
...
untersuchten Systeme lediglich ein \enquote{richtig} oder \enquote{falsch} zulässt, spielt dieser Punkt keine Rolle.

	
	\item \textbf{Functional Appropriateness}: Grad, zu welchem die Funktionalitäten der Software die Erfüllung der Aufgaben ermöglichen und unterstützen.
...
Kriterium nicht betrachtet werden soll.
	...
	\item \textbf{Operability}: Beschreibt Attribute, die ein System besitzt, deren Zweck es ist die Bedienung zu vereinfachen.
...
Nutzer gegen eigene Fehler (z.B. während der Eingabe) schützt.
...
durchgeführten Arbeiten, soll dieser Aspekt nicht gezielt betrachtet werden. Sollten Auffälligkeiten in diesem Bereich auftreten, werden
...
Komponenten, die verändert werden sollen, sowie die Diagnose von
...
wenig bis keine Möglichkeiten für Testing und setzt mehr auf zusätzliche
...
Terraform Kommandos werden in der Bash ausgeführt.
...
Die Abhängigkeitsgraphen der jeweiligen Systeme in Kapitel 3.2.3 und 3.2.4 werden mit dem in Terraform integrierten Kommando \texttt{terraform graph} erstellt und mithilfe des Tools \textbf{Terraform Graph Beautifier} visuell aufbereitet. Abb. 3.2 bzw. 3.3 stellen jeweils ein Beispiel für einen \enquote{rohen} und den zugehörigen aufbereiteten Graphen dar.

\begin{figure}[H]
	
	\includegraphics[keepaspectratio, height=3cm]{fig/aa-hauptteil/tf-graph.png}
	\centering
	
	\caption{Output Terraform Graph[Eigene Abbildung]}
	
	\centering
	
\end{figure}

In der Darstellung von \texttt{terraform graph} werden sämtliche Abhängigkeiten und Elemente inklusive der Module und Provider dargestellt.

\begin{figure}[H]
	
	\includegraphics[keepaspectratio, height=6cm]{fig/aa-hauptteil/tf-graph-beautified.png}
	\centering
	
	\caption{Output Terraform Graph Beautifier[Eigene Abbildung]}
	
	\centering
	
\end{figure}

Terraform Graph Beautifier versteckt einige Elemente, wie die Provider, um eine gesamtheitlich bessere Übersicht verschaffen zu können. Module werden als farblich hinterlegte Rechtecke dargestellt, innerhalb derer sich die zugehörigen Ressourcen befinden. Die generierte Datei ist dabei eine HTML-Datei, die es erlaubt alle Elemente durch Drag-and-Drop zu verschieben, um die Darstellung individuell anpassen zu können. Der Nachteil dabei ist, das dies auch erforderlich ist; die standardmäßige Ausgabe achtet nicht auf eine sinnvolle Anordnung der Elemente.

\subsection{High-Level Aufbau des Testsystems}

Als primäres Testobjekt dient ein Infrastruktur-System, das typische Anwendungsfälle
...
\texttt{Platform}, das die gesamte Infrastruktur enthält. Für diese Arbeit wurden
...
\begin{figure}[H]
	...
	\caption{Infrastructure Showcase High-level Struktur[Eigene Abbildung]}
...
\begin{landscape}
	...
	\begin{figure}[H]
		...
		\caption{Testsystem in Microsoft Azure[Eigene Abbildung]}
...
Die beiden Module für den Kubernetes Cluster (\texttt{azurerm\_kubernetes\_cluster.jsa\_k8s}) und den Datenbankserver (\texttt{azurerm\_postgresql\_server.pgdatabaseserver}) sowie das Root-Modul werden in Abb. 3.5 mit allen Terraform Ressourcen und deren Abhängigkeiten sowie allen Variablen und Outputs dargestellt.

Die erste Auffälligkeit dabei ist, dass nicht alle Ressourcen direkt voneinander abhängig sind. Das Kubernetes Deployment, bestehend aus der Applikation\\(\texttt{kubernetes\_deployment.jsa\_testapp})\footnote{Die Testapp ist ein trivialer Node.js Server} und einem Loadbalancer\\ (\texttt{kubernetes\_service.jsa\_testapp\_lb}), ist als Terraform Ressource zum Beispiel nicht direkt vom Kubernetes Cluster abhängig, stattdessen wird der Kubernetes Terraform Provider mit der Adresse des Clusters als Deployment-Ziel konfiguriert.

Weiterhin ist erkennbar, dass so gut wie alle Ressourcen von der \texttt{azurerm\_resource\_group} abhängig sind, Google Cloud Platform verwendet hier ein anderes Konzept, Näheres wird dazu im nächsten Unterkapitel beschrieben. Gleiches gilt für den Secret Storage, dieser wird hier mit der Ressource \texttt{azurerm\_key\_vault.jsa\_vault} erstellt.

Direkter Zugriff auf den Datenbankserver ist durch dessen Fully Qualified Domain Name (\texttt{output.postgres\_server\_fqdn}) möglich, User und Passwort werden im Secret Storage gespeichert.

Die Container Registry wird in Form der Data Source\\ \texttt{data.azurerm\_container\_registry.containerregistry} eingebunden, Berechtigungen, die notwendig, sind um das Kubernetes-Deployment durchzuführen, werden durch das\\ \texttt{azurerm\_role\_assignment.jsa\_acrpull\_role} vergeben.
...
\begin{landscape}
	...
	\begin{figure}[H]
		...
		\includegraphics[keepaspectratio, height=8.5cm]{fig/aa-hauptteil/gcp_konkreter_aufbau.png}

		\caption{Testsystem in Google Cloud Platform[Eigene Abbildung]}
...
Die beiden zentralen Elemente der Module sind äquivalent zu Azure der Kubernetes Cluster \texttt{google\_container\_cluster.jsa\_showcase\_gke} und der Datenbankserver\\ \texttt{google\_sql\_database\_server.pgdatabaseserver1}. Anders als bei Azure existiert keine dedizierte Ressource für eine PostgreSQL-Server, die \enquote{generische} Ressource wird per Attribut entsprechend konfiguriert.

Wie bereits im vorangegangen Unterkapitel angesprochen, existiert in diesem Aufbau für Google Cloud Platform keine zur Resourcegroup äquivalente Terraform Ressource. Stattdessen muss ein Google Cloud Platform Project mit einer einzigartige Id angelegt werden, im Anschluss wird der Google Terraform Provider für dieses Projekt konfiguriert. Gleiches gilt für den Secret Storage, dieser existiert projekt-weit und es muss lediglich der entsprechende Service im Projekt aktiviert werden.

Direkter Zugriff auf den Datenbankserver wird hier per Ip-Adresse ermöglicht\\ (\texttt{output\_postgres\_server\_ipv4}), anders als in Azure ist auch der Zugriff auf den Endpoint des Kubernetes Cluster sehr simpel identifizierbar.

Die Container Registry ist nicht unbedingt auf den ersten Blick als diese erkennbar. Ein einfacher Storage Bucket in Form der Data Source\\ \texttt{data.google\_storage\_bucket.container\_registry\_storage\_bucket} wird hier als Container Registry verwendet, warum dies der Fall ist, wird in der Bewertung der Functional Completeness (Kapitel 4.2) genauer erläutert.
...
Eine weiterer kritischer Aspekt ist, das zeitliche Verhalten
...
\begin{itemize}
	...
	\item Das Test-Infrastruktur-System
...
Jeder Test wird drei mal wiederholt. Die Messungen werden nur durchgeführt,
sofern die Plattform den Status der betroffenen Dienste als Normal/OK
führt. 

\subsubsection{Recoverability}

Für diesen Test wird zunächst der Aufbau des Systems
durch das gewohnte \texttt{terraform apply} gestartet. Nach 
	...
	\begin{table}[H]
		\begin{tabular}{@{}lll@{}}
			...
			\begin{tabular}[c]{@{}l@{}}Funktionalität wird durch die Ressource 
				...
				Erstellung nicht wie erwünscht definiert \\ werden. Default ist 
...
Collation sind zum Zeitpunkt des Erstellens nicht möglich.
Das Charset entspricht dem gewünschten Wert, die Collation jedoch
nicht.\\
Der Secret Storage wird auf GCP einmal für ein GCP Projekt angelegt,
Azure hingegen bietet die Möglichkeit mehrere Key Vaults innerhalb
einer Resource Group anzulegen. Die Speicherung des tatsächlichen Secrets
in der Ressource\\ \texttt{google\_secret\_manager\_seceret\_version} anstelle
des zuvor anzulegenden\\ \texttt{google\_secret\_manager\_seceret} fällt dadurch wenig intuitiv aus.\\
...
eines Container Images in GCP stellt die Artifact Registry dar,
...
Eine weitere mögliche Ursache für die Zeitdifferenz beim Aufbau
...
Google Cloud Platform und der Azure Location \enquote{westeurope}
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lll}
			...
			\begin{tabular}[c]{@{}l@{}}Terraform State 
...
Die Tests zur Recoverability vermitteln ein einheitliches Bild: Das Ergebnis jedes Szenarios ist die Korrumpierung
...
zusätzliche Mechanismen zur Fehlerprävention beim Deployment
...
Der größte Unterschied der die Durchführung betreffend auffiel, ist
...
von GCP über das Zerstören und Neuerstellen der Ressource möglich
