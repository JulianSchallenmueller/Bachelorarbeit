...
Zu diesem Zweck wird ein Beispielszenario, bestehend aus einem Kubernetes Cluster und einem Datenbankserver, für beide Plattformen in Terraform Code implementiert und auf diesen deployed. Zusätzlich wird im Vorfeld manuell eine Container Registry auf den beiden Plattformen erstellt und ein Image auf dieser bereitgestellt. Diese Container Registry soll dann als Terraform Data Source im Projekt eingebunden und das darauf liegende Image via Terraform auf dem Kubernetes Cluster deployed werden.\\
...
Die beiden Module für den Kubernetes Cluster (\texttt{azurerm\_kubernetes\_cluster.jsa\_k8s}) und den Datenbankserver (\texttt{azurerm\_postgresql\_server.pgdatabaseserver}) sowie das Root-Modul werden in Abb. 3.3 mit allen Terraform Ressourcen und deren Abhängigkeiten sowie allen Variablen und Outputs dargestellt.
